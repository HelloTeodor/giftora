          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
            <div className="text-center lg:text-left">
              ...
              <p className="text-navy-300 text-sm">
                Exclusive offers, new arrivals, and gifting ideas.
              </p>
            </div>
      ...
      <div className="section-padding py-12">
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
          ...
          <div className="col-span-2 md:col-span-4 lg:col-span-2">
            <Link href="/" className="block mb-4">
              <span className="font-playfair text-2xl font-bold tracking-[0.04em] text-white">
                Happy<span className="text-gold-500">Box</span>
              </span>
              <span className="block text-[10px] italic text-navy-400 font-light tracking-wide mt-0.5">
                Thoughtful Gifts &amp; Living
              </span>
            </Link>
            <p className="text-navy-300 text-sm leading-relaxed mb-6 max-w-xs">
              Beautifully curated, eco-friendly gift boxes for every occasion —
              ...
              <ul className="space-y-2.5">
                {links.map((l, idx) => (
                  <li key={`${title}-${idx}`}>
                    <Link
                      href={l.href}
                      className="text-navy-300 text-sm hover:text-gold-400 transition-colors"
                    >
                      {l.label}
                    </Link>
                  </li>
        ...
        <div className="mt-10 pt-8 border-t border-navy-800 flex flex-col sm:flex-row gap-4 sm:gap-8">
          <a
            ...
            className="flex items-center gap-2 text-navy-300 text-sm hover:text-gold-400 transition-colors"
          >
        ...
        <div className="section-padding py-5 flex flex-col sm:flex-row items-center justify-between gap-3 text-sm text-navy-400">
          ...
          <div className="flex items-center gap-2">
            {["Visa", "MC", "Amex", "PayPal", "Klarna"].map((p) => (
              <span
                key={p}
                className="px-2 py-1 bg-navy-800 text-navy-300 text-xs rounded font-medium"
              >
                {p}
              </span>
            ))}
